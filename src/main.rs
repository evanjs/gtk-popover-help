...
use gio::prelude::*;
use gtk::{ApplicationWindow, Builder, ButtonExt, GtkWindowExt, Inhibit, ListBox, ListBoxExt, PopoverExt, PopoverMenu, WidgetExt};
use gtk::prelude::*;
use glib::ParamSpec;
...
impl AppCtx {
  pub fn new(application: &gtk::Application) -> Self {
    ...
    let props: Vec<ParamSpec> = builder.list_properties();
    for p in props {
      let ps: ParamSpec = p;
      dbg!(ps.get_name());
      dbg!(ps.get_default_value());
    }
  ...
  pub fn setup_button_one_clicked(
    self) {
    let ctx = self.clone();
    ctx.search_buttons
      .get_button_one()
      .connect_clicked(move |s| {
  ...
  pub fn setup_button_two_clicked(
    self) {
    let ctx = self.clone();
    ctx.search_buttons
      .get_button_two()
  ...
  pub fn setup_button_three_clicked(
    self
  ) {
    let ctx = self.clone();
    ctx.search_buttons
  ...
  fn setup_meta_list(self) {
    self.clone().list_box.connect_button_press_event(
      move |list_box: &ListBox, event_button: &EventButton| {
        self.clone().connect_mouse_events(
          list_box.clone(),
          event_button.clone(),
        )
      },
